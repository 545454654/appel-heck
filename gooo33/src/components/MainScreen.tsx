        ...
        {selectedGame === 'apple' ? (
          <>
            ...
            <div className="p-3 glass rounded-[1.75rem] border-green-500/20 space-y-3">
              ...
              {/* 10 Rows with targetRows order (from row 9 at top to row 0 at bottom) - Explicit LTR for Casino Layout */}
              <div className="flex flex-col gap-1 items-center w-full" dir="ltr">
                {/* Column header indicators 1-5 */}
                <div className="flex items-center justify-between w-full max-w-[420px] px-1.5 opacity-60">
                  <div className="min-w-[52px] text-[8px] font-mono font-bold text-center text-white/40">
                    ODDS
                  </div>
                  <div className="grid grid-cols-5 gap-1.5 flex-1 justify-items-center">
                    {[1, 2, 3, 4, 5].map((colNum) => (
                      <span key={colNum} className="text-[9px] font-mono font-bold text-green-400/80">
                        {colNum}
                      </span>
                    ))}
                  </div>
                ...
                {targetRows.map((rowInfo, rIdx) => (
                  ...
                  >
                    {/* Odds Multiplier Badge on the LEFT */}
                    ...
                    {/* 5 Column Cells (Columns 1 -> 5 from Left to Right) */}
                    <div className="grid grid-cols-5 gap-1.5 flex-1 justify-items-center">
